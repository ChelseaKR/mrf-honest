...
# Read rather than retyped: the per-column rules are generated from these two tuples, so a
# column added to either one gets a test case without anyone remembering to write one.
...
#
# Twenty-one CSV rules were catalogued and tripped by nothing. Eighteen of them are the
# per-column family, which is generated by one loop over the module's own required-column
# tuples -- so the parametrization reads those tuples rather than retyping them, and a column
# added to either tuple gets a case without anyone remembering to write one.


@pytest.mark.parametrize("column", [*_SHARED_REQUIRED_COLUMNS, *_TALL_REQUIRED_COLUMNS])
def test_every_required_charge_column_is_reported_when_it_is_absent(
    tmp_path: Path, column: str
) -> None:
    header = ",".join(name for name in TALL_HEADER.split(",") if name != column)
    assert header != TALL_HEADER, f"{column!r} is not a column of TALL_HEADER"
    path = _write(tmp_path, GENERAL_HEADER, GENERAL_VALUES, header, TALL_DOLLAR_ROW)

    inspection = inspect_hospital_csv_file(path, as_of=AS_OF)

    assert f"CMS_CSV_COLUMN_{_column_code(column)}_MISSING" in _codes(inspection)
    assert column in inspection.missing_columns


def test_a_data_row_with_codes_and_charges_but_no_description_is_reported(
    tmp_path: Path,
) -> None:
    inspection = _tall(tmp_path, TALL_DOLLAR_ROW.replace("MRI brain,", ",", 1))

    assert "CMS_CSV_DESCRIPTION_MISSING" in _codes(inspection)


def test_a_drug_type_of_measurement_outside_the_accepted_set_is_reported(
    tmp_path: Path,
) -> None:
    row = TALL_DOLLAR_ROW.replace(
        "MRI brain,70551,CPT,,outpatient,,,", "MRI brain,70551,CPT,,outpatient,10,furlongs,", 1
    )
    assert row != TALL_DOLLAR_ROW

    inspection = _tall(tmp_path, row)

    assert "CMS_CSV_DRUG_TYPE_INVALID" in _codes(inspection)


def test_algorithm_encoded_payer_rates_are_counted_separately(tmp_path: Path) -> None:
    row = (
        "MRI brain,70551,CPT,,outpatient,,,1200,900,Acme Health,PPO,,,base + implant,"
        "75,50,100,12,case rate,700,950,"
    )

    inspection = _tall(tmp_path, row)

    assert inspection.algorithm_rate_count == 1
    assert "CSV_INTERPRETABILITY_ALGORITHM_RATES" in _codes(inspection)
