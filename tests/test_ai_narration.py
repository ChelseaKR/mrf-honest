...
RECORDS = [
    json.loads(line)
    for line in (ROOT / "data" / "scorecards.jsonl").read_text(encoding="utf-8").splitlines()
    if line.strip()
]
...
def test_eval_scores_records_and_records_provenance(tmp_path: Path) -> None:
    ...
    meta = eval_module.metadata(provider, ROOT, ROOT / "data" / "scorecards.jsonl")
    ...
    assert len(eval_module.load_records(ROOT / "data" / "scorecards.jsonl", limit=3)) == 3
...
def test_narrate_cli_prints_claims_and_validates_index(
    ...
    records = ROOT / "data" / "scorecards.jsonl"
